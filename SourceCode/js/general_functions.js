...
function enable_move(checker, cell, settings) {
    ...
    if (need_beat_arr.length > 0) {
            ...
            if (need_beat_arr[x].from == get_array_id_by_ij(checker_i, checker_j, settings))
                for (var y = 0; y < need_beat_arr[x].to.length; y++)
                    if (need_beat_arr[x].to[y] == get_array_id_by_ij(cell_i, cell_j, settings)) {
                        ...
                        //mark beaten
                        var pos_i1 = (checker_i < cell_i) ? checker_i : cell_i; var pos_i2 = (checker_i > cell_i) ? checker_i : cell_i;
                        var pos_j1 = (checker_j < cell_j) ? checker_j : cell_j; var pos_j2 = (checker_j > cell_j) ? checker_j : cell_j;
                        for (var i = pos_i1 + 1, j = pos_j1 + 1; i <= pos_i2 - 1, j <= pos_j2 - 1; i++, j++) {

                            var beaten_mark = $("<div />").addClass(settings.checker_beaten_mark_class);

                            $("." + settings.checker_class + "[i=" + i + "][j=" + j + "]").append(beaten_mark).attr("beaten", "true");
                        }
        ...
        case settings.board_dictionary.white_king:
            {
                break;
            }
        case settings.board_dictionary.black_king:
            {
                break;
            }
...
function need_beat(checker, only_this/*true if checker start beat and neet to beat several checkers */, settings) {
    var cells = parseInt(settings.cells, 10);
...
function beat(from_array_id, to_array_id, checker, settings) {

};

function switch_move(checker, settings) {
    settings.current_checkers_move = (settings.current_checkers_move == 'white') ? 'black' : 'white';
    return settings;
};